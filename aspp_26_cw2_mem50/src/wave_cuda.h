...
struct CudaWaveSimulation : WaveSimulation {
    ...
    // Initialise by taking ownership of a CPU state (skip_cpu memory-optimised path).
    static CudaWaveSimulation from_cpu_sim(fs::path const& cp, WaveSimulation&& source);
