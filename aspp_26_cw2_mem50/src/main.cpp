...
bool skipcpu_memopt_enabled_from_env() {
    const char* env = std::getenv("AWAVE_SKIPCPU_MEMOPT");
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        bool use_skipcpu_memopt = rp.skip_cpu && skipcpu_memopt_enabled_from_env();
        if (rp.skip_cpu) {
            outroot(rank, "skip_cpu memory optimisation is {}", use_skipcpu_memopt ? "enabled" : "disabled");
        }

        if (use_skipcpu_memopt && !rp.doIO) {
            // sos is not needed for CUDA setup (only cs2/damp/u are consumed).
            cpu_state.sos = array3d{};
        }

        GpuWaveSimulation gpu_state;
        if (use_skipcpu_memopt) {
            gpu_state = GpuWaveSimulation::from_cpu_sim(gpu_cp, std::move(cpu_state));
            cpu_state = CpuWaveSimulation{};
        } else {
            // Baseline path for A/B comparison.
            gpu_state = GpuWaveSimulation::from_cpu_sim(gpu_cp, cpu_state);
        }

        if (rp.cpu_serial && !rp.skip_cpu) {
          cpu_state = CpuWaveSimulation();
        ...
        // Timing stats
        int active_time = rp.skip_cpu ? gpu_state.u.time() : cpu_state.u.time();
        auto n_chunks = ceildiv(p.nsteps - active_time, p.out_period);
