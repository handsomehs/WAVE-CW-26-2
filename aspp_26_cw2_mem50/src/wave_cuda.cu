...
CudaWaveSimulation CudaWaveSimulation::from_cpu_sim(const fs::path& cp, WaveSimulation&& source) {
    CudaWaveSimulation ans;
    auto rank = source.decomp.rank;
    outroot(rank, "Initialising {} simulation by taking ownership of {}...", ans.ID(), source.ID());
    ans.params = source.params;
    ans.decomp = source.decomp;
    ans.checkpoint = cp;

    bool source_has_io = static_cast<bool>(source.h5);

    int count = 0;
    auto err = cudaGetDeviceCount(&count);
    if (err != cudaSuccess || count == 0) {
        // Preserve CPU fallback behaviour when no CUDA device is available.
        ans.u = source.u.clone();
        ans.sos = source.sos.clone();
        ans.cs2 = source.cs2.clone();
        ans.damp = source.damp.clone();
    ...
    ans.u = std::move(source.u);
    ans.sos = std::move(source.sos);
    ans.cs2 = std::move(source.cs2);
    ans.damp = std::move(source.damp);
    ...
    // Once device data has been created, these host coefficient fields are no
    // longer needed for CUDA execution in skip_cpu mode.
    ans.sos = array3d{};
    ans.cs2 = array3d{};
    ans.damp = array3d{};
