...
## 7) Start Nsight profiling runs (NSYS only)
...
- `logs/profiles/nsys/` (`.nsys-rep`, `.sqlite`, plus run JSON)

NCU profiling has been removed from the default workflow. The project now uses Nsight Systems timing + manual
derivation for roofline-style evidence because this is robust under cluster counter restrictions.

### 7.1 H100 roofline proxy with Nsight Systems + manual derivation

This is the standard path for single-GPU roofline evidence (1A/1B/1C).

Submit fallback job:

```bash
cd /home/eidf018/eidf018/shared/s2792840-epcc-pvc/ASPP_CW2/aspp_26_cw2
kgpu create -f jobs/run-cw2-profile-nsys-h100-roofline-fallback-user.yml
kubectl get jobs | grep awave-cw2-nsys-h100-roofline-manual
```

What this job does:

- Runs 1-GPU H100 cases with `nsys profile` for sizes `256`, `512`, `1000`, `2000`.
- Uses `--trace=cuda,nvtx --cuda-event-trace=kernel` and exports `cuda_gpu_kern_sum` + `cuda_kern_exec_trace` CSV.
- Computes 1A/1B/1C report tables from NSYS timing only:
  - 1A: interior kernel time, SU/s, inferred B/site, naive/optimistic bandwidth utilization.
  - 1B: boundary kernels vs interior share at size `1000`.
  - 1C: multi-scale trend table for `256/512/1000/2000`.

Manual derivation formulas used in the summary:

```text
N = (size - 2)^3
SU/s = N / t_interior
B/site (inferred) = B_HBM_peak / (SU/s)
U_naive = SU/s * 88 / B_HBM_peak
U_optimistic = SU/s * 40 / B_HBM_peak
```

Expected artifacts under `logs/profiles/nsys_roofline_h100/`:

- `nsys_single_<size>.nsys-rep`
- `nsys_single_<size>_stats_cuda_gpu_kern_sum.csv`
- `nsys_single_<size>_stats_cuda_kern_exec_trace.csv`
- `summary.json`
- `summary.md`

Notes:

- If `U_naive > 1.0` (or `>100%`), that is direct evidence that cache reuse is required.
- `summary.md` is structured to map directly into report sections 1A/1B/1C.
