...
DIR_RE = re.compile(r"awave-cw2-boundary-ab-(a100|h100)-(\d+)g-(pq|uq)(?:-r(\d+))?$")
...
def decide_group(summary, args):
    ...
    queue_repeat_min = min((q["repeats"] for q in summary["queue_stats"]), default=0)
    qspread_reliable = len(summary["queue_stats"]) > 1 and queue_repeat_min >= args.min_repeats_per_queue_for_spread
    ...
    if pairs < args.min_pairs:
        ...
        if qspread_reliable and qspread > args.max_queue_spread_pct:
            status = "needs-retest"
    ...
    if summary["gpus"] == 2:
        if status in ("promote", "provisional-promote"):
            action = "enable-boundary-and-continue-next-optimizations"
        elif status in ("drop", "provisional-drop", "no-promote", "provisional-no-promote"):
            action = "keep-boundary-off-and-move-next-optimization"
        else:
            action = "run-more-2g-repeats-before-keep-or-drop"
    else:
        if status in ("promote", "provisional-promote"):
            action = "enable-boundary-for-this-gpu-count"
        elif status in ("drop", "provisional-drop", "no-promote", "provisional-no-promote"):
            action = "keep-boundary-off-for-this-gpu-count"
        else:
            action = "collect-more-runs"

    summary["pairs"] = pairs
    summary["queue_spread_pct"] = qspread
    summary["queue_spread_reliable"] = qspread_reliable
...
def write_markdown(path: Path, case_summaries, group_summaries, args):
    lines = []
    lines.append("# Boundary Split Gate Summary")
    lines.append("")
    ...
    lines.append(f"- min_repeats_per_queue_for_spread: `{args.min_repeats_per_queue_for_spread}`")
    lines.append("")
    ...
    lines.append("")
    lines.append("| gpu | gpus | gain_steady_pct | pairs | queue_spread_pct | spread_reliable | status | recommended_action |")
    lines.append("|---|---:|---:|---:|---:|---|---|---|")
    for row in sorted(group_summaries, key=lambda r: (r["gpu"], r["gpus"])):
        lines.append(
            "| {gpu} | {gpus} | {gain} | {pairs} | {spread} | {reliable} | {status} | {action} |".format(
                gpu=row["gpu"],
                gpus=row["gpus"],
                gain=fmt(row["gain_steady_pct"]),
                pairs=row["pairs"],
                spread=fmt(row["queue_spread_pct"]),
                reliable=("yes" if row.get("queue_spread_reliable") else "no"),
                status=row["status"],
...
def parse_args():
    p = argparse.ArgumentParser(description="Decide boundary promote/drop from A/B JSON logs")
    p.add_argument("--logs-root", type=Path, default=Path("logs"))
    p.add_argument("--dir-glob", type=str, default="awave-cw2-boundary-ab-*")
    p.add_argument("--json-out", type=Path, default=Path("logs/boundary_gate_summary.json"))
    p.add_argument("--md-out", type=Path, default=Path("logs/boundary_gate_summary.md"))
    ...
    p.add_argument("--min-repeats-per-queue-for-spread", type=int, default=2)
    return p.parse_args()
...
def main():
    ...
    if not case_summaries:
        raise SystemExit("No boundary case directories with boundary0/boundary1 JSON found")
    ...
    payload = {
        "thresholds": {
            ...
            "min_repeats_per_queue_for_spread": args.min_repeats_per_queue_for_spread,
        },
