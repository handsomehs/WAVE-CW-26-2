...
STATUS_MD="logs/mpistack_dualqueue_repeats_status.md"
RUN_LOG="logs/mpistack_dualqueue_repeats.log"

usage() {
  cat <<'EOF'
Usage: tools/run_mpistack_dualqueue_repeats.sh [--repeats N] [--pairs a100-2g,h100-2g] [--namespace eidf018ns]

Behavior:
- For each pair/repeat, submit profiling-queue and user-queue jobs simultaneously.
- As soon as one queue produces valid stack_default/stack_ob1 JSON outputs, delete the sibling job from the other queue.
...
declare -A MANIFEST_PQ=(
  [a100-2g]="jobs/run-cw2-mpistack-ab-a100-2gpu-profiling.yml"
  [h100-2g]="jobs/run-cw2-mpistack-ab-h100-2gpu-profiling.yml"
)
declare -A MANIFEST_UQ=(
  [a100-2g]="jobs/run-cw2-mpistack-ab-a100-2gpu-user.yml"
  [h100-2g]="jobs/run-cw2-mpistack-ab-h100-2gpu-user.yml"
)
declare -A OUTDIR_PQ=(
  [a100-2g]="logs/awave-cw2-mpistack-ab-a100-2g-pq"
  [h100-2g]="logs/awave-cw2-mpistack-ab-h100-2g-pq"
)
declare -A OUTDIR_UQ=(
  [a100-2g]="logs/awave-cw2-mpistack-ab-a100-2g-uq"
  [h100-2g]="logs/awave-cw2-mpistack-ab-h100-2g-uq"
)
...
out_dir = sys.argv[1]
for fname in ("stack_default.json", "stack_ob1.json"):
...
init_status_md() {
  {
    echo "# Dual-Queue MPI Stack Repeats"
    echo
...
run_one_repeat() {
  ...
  render_manifest "$base_pq" "$tmp_pq" "$outbase_pq" "$outdir_pq" "awave-cw2-mpistack-ab-${pair}-pq-r${rep}"
  render_manifest "$base_uq" "$tmp_uq" "$outbase_uq" "$outdir_uq" "awave-cw2-mpistack-ab-${pair}-uq-r${rep}"
