...
DIR_RE = re.compile(r"awave-cw2-overlap-ab-(a100|h100)(?:-(\d+)g)?-(pq|uq)(?:-r(\d+))?$")
...
def parse_case_dir(path: Path):
    ...
    if gpus is None:
        # Legacy naming in this repo: h100-pq/uq means 8-GPU overlap job.
        gpus = 8 if gpu == "h100" else None
    else:
        gpus = int(gpus)
    if gpus is None:
        return None
    return {"gpu": gpu, "gpus": gpus, "queue": queue, "repeat": int(repeat) if repeat else None}
...
def decide_group(summary, args):
    ...
    if summary["gpus"] == 2:
        if status in ("promote", "provisional-promote"):
            action = "queue-4g-8g-and-continue-next-optimizations"
        elif status == "drop":
            action = "cancel-overlap-optimization"
        elif status in ("no-promote", "provisional-no-promote"):
            action = "stop-overlap-campaign-and-move-to-next-optimization"
        else:
            action = "run-more-2g-repeats-before-keep-or-drop"
    else:
        if status in ("promote", "provisional-promote"):
            action = "keep-overlap-on-for-this-gpu-count"
        elif status == "drop":
            action = "use-overlap-off-for-this-gpu-count"
        elif status in ("no-promote", "provisional-no-promote"):
            action = "stop-overlap-campaign-for-this-gpu-count"
        else:
...
def write_markdown(path: Path, case_summaries, group_summaries, args):
    lines = []
    lines.append("# Overlap Gate Summary")
    lines.append("")
...
def parse_args():
    p = argparse.ArgumentParser(description="Decide overlap promote/drop from A/B JSON logs")
    p.add_argument("--logs-root", type=Path, default=Path("logs"))
    p.add_argument("--dir-glob", type=str, default="awave-cw2-overlap-ab-*")
    p.add_argument("--json-out", type=Path, default=Path("logs/overlap_gate_summary.json"))
    p.add_argument("--md-out", type=Path, default=Path("logs/overlap_gate_summary.md"))
...
def main():
    ...
    if not case_summaries:
        raise SystemExit("No overlap case directories with overlap0/overlap1 JSON found")
