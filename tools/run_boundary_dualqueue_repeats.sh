...
STATUS_MD="logs/boundary_dualqueue_repeats_status.md"
RUN_LOG="logs/boundary_dualqueue_repeats.log"

usage() {
  cat <<'EOF'
Usage: tools/run_boundary_dualqueue_repeats.sh [--repeats N] [--pairs a100-2g,h100-2g,a100-4g,h100-8g] [--namespace eidf018ns]

Behavior:
- For each pair/repeat, submit profiling-queue and user-queue jobs simultaneously.
- As soon as one queue produces valid boundary0/boundary1 JSON outputs, delete the sibling job from the other queue.
...
declare -A MANIFEST_PQ=(
  [a100-2g]="jobs/run-cw2-boundary-ab-a100-2gpu-profiling.yml"
  [h100-2g]="jobs/run-cw2-boundary-ab-h100-2gpu-profiling.yml"
  [a100-4g]="jobs/run-cw2-boundary-ab-a100-4gpu-profiling.yml"
  [h100-8g]="jobs/run-cw2-boundary-ab-h100-8gpu-profiling.yml"
)
declare -A MANIFEST_UQ=(
  [a100-2g]="jobs/run-cw2-boundary-ab-a100-2gpu-user.yml"
  [h100-2g]="jobs/run-cw2-boundary-ab-h100-2gpu-user.yml"
  [a100-4g]="jobs/run-cw2-boundary-ab-a100-4gpu-user.yml"
  [h100-8g]="jobs/run-cw2-boundary-ab-h100-8gpu-user.yml"
)
declare -A OUTDIR_PQ=(
  [a100-2g]="logs/awave-cw2-boundary-ab-a100-2g-pq"
  [h100-2g]="logs/awave-cw2-boundary-ab-h100-2g-pq"
  [a100-4g]="logs/awave-cw2-boundary-ab-a100-4g-pq"
  [h100-8g]="logs/awave-cw2-boundary-ab-h100-8g-pq"
)
declare -A OUTDIR_UQ=(
  [a100-2g]="logs/awave-cw2-boundary-ab-a100-2g-uq"
  [h100-2g]="logs/awave-cw2-boundary-ab-h100-2g-uq"
  [a100-4g]="logs/awave-cw2-boundary-ab-a100-4g-uq"
  [h100-8g]="logs/awave-cw2-boundary-ab-h100-8g-uq"
)
...
out_dir = sys.argv[1]
for fname in ("boundary0.json", "boundary1.json"):
...
init_status_md() {
  {
    echo "# Dual-Queue Boundary Split Repeats"
    echo
...
run_one_repeat() {
  ...
  render_manifest "$base_pq" "$tmp_pq" "$outbase_pq" "$outdir_pq" "awave-cw2-boundary-ab-${pair}-pq-r${rep}"
  render_manifest "$base_uq" "$tmp_uq" "$outbase_uq" "$outdir_uq" "awave-cw2-boundary-ab-${pair}-uq-r${rep}"
